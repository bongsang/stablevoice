...
from moviepy.editor import AudioFileClip, AudioClip, CompositeVideoClip, concatenate_audioclips
...
# [(start_time, audio_clip), ...

def concatenate_audio_clips(audio_clips):
    # Sort clips by start time
    sorted_clips = sorted(audio_clips, key=lambda x: x[0])
    final_clips = []

    # Initialize the current end of the last clip processed
    last_end = 0

    for start, clip in sorted_clips:
        if start < last_end:
            # If there's overlap, handle accordingly, perhaps by trimming
            clip = clip.set_start(start).set_end(last_end)
        else:
            # Fill the gap with silence if there is any
            silence_duration = start - last_end
            if silence_duration > 0:
                silence_clip = generate_silent_audio(silence_duration)
                silence_clip = silence_clip.set_start(last_end)
                final_clips.append(silence_clip)
        final_clips.append(clip.set_start(start))
        last_end = max(last_end, start + clip.duration)

    # Combine all clips into one final clip
    final_audio = concatenate_audioclips([clip[1] for clip in sorted_clips])
    final_output_path = output_dir / "final_audio.mp3"
    final_audio.write_audiofile(final_output_path.as_posix())
...
concatenate_audio_clips(audio_clips)
